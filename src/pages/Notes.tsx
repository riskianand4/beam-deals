...
const Notes = () => {
  ...
  const [confirmAdminDelete, setConfirmAdminDelete] = useState<string | null>(
    null,
  );
  ...
  // For admin/access: tabs & employee dropdown
  const [notesTab, setNotesTab] = useState<"pribadi" | "karyawan">("pribadi");
  ...
  useEffect(() => {
    if (canManageNotes && employeeId) {
      ...
      canManageNotes && !employeeId && notesTab === "karyawan" &&
      selectedEmployeeId
    ) {
      // Admin viewing employee notes via dropdown
      api.getAdminNotes({ toEmployeeId: selectedEmployeeId }).then(
        setAdminNotes,
      ).catch(() => {});
  ...
  const allBoardNotes:
    ((DailyNote & { _type: "daily" }) | (AdminNote & { _type: "admin" }))[] = [
  ...
  const displayedNotes = filterMode === "all"
    ...
    : allBoardNotes.filter((n) => {
      if (
        n._type === "daily"
      ) return (n as DailyNote & { _type: "daily" }).date === currentDate;
  ...
  // ============================================================
  // VIEW: ADMIN/ACCESS — Tabs: Pribadi & Karyawan
  // ============================================================
  if (canManageNotes && !employeeId && notesTab === "karyawan") {
    const empNotes = adminNotes.filter((n) =>
      n.toEmployeeId === selectedEmployeeId
    );
    ...
    return (
      ...
      >
        ...
        <Tabs
          value={notesTab}
          onValueChange={(v) => {
            setNotesTab(v as "pribadi" | "karyawan");
            setAdminNotes([]);
            setDailyNotes([]);
          }}
        >
          <TabsList className="relative">
            ...
            >
              {/* Tanpa animasi di sini */}
              <span className="relative z-10 flex items-center gap-1.5">
                <StickyNote className="w-3.5 h-3.5" /> Pribadi
              </span>
            </TabsTrigger>
            ...
            <TabsTrigger
              ...
              className="relative text-xs data-[state=active]:bg-transparent data-[state=active]:shadow-none"
            >
              {/* Animasi langsung ditaruh di Karyawan */}
              <motion.div
              ...
              />
              <span className="relative z-10 flex items-center gap-1.5">
                <Send className="w-3.5 h-3.5" /> Karyawan
              </span>
            </TabsTrigger>
          </TabsList>
        </Tabs>
              ...
              {filteredEmployees.map((emp) => (
                <SelectItem key={emp.id} value={emp.id} className="text-xs">
                  {emp.name} {emp.position
                    ? <Badge className=" ">{emp.position}</Badge>
                    : ""}
                </SelectItem>
              ))}
            </SelectContent>
                  ...
                  ? (
                    <div className="ms-card">
                      <EmptyState
                        ...
                        description="Catatan yang kamu kirim akan muncul di sini."
                        compact
                      />
                    </div>
                  )
          ...
          : (
            <div className="ms-card">
              <EmptyState
                ...
                description="Pilih karyawan dari dropdown di atas untuk mengirim atau melihat catatan."
                compact
              />
            </div>
          )}
        ...
        <ConfirmDialog
          ...
          onOpenChange={(open) => {
            if (!open) setConfirmAdminDelete(null);
          }}
          ...
          variant="destructive"
          onConfirm={() =>
            confirmAdminDelete && handleAdminDeleteNote(confirmAdminDelete)}
        />
      </motion.div>
    );
  }
  ...
  const doAddNote = async () => {
    ...
    try {
      const targetDate = filterMode === "date"
        ? currentDate
        : format(new Date(), "yyyy-MM-dd");
          ...
          >
            <TabsList className="relative">
              <TabsTrigger
                ...
                className="relative text-xs data-[state=active]:bg-transparent data-[state=active]:shadow-none"
              >
                {/* Animasi langsung ditaruh di Pribadi */}
                <motion.div
                  layoutId="tab-indicator"
                  className="absolute inset-0 rounded-sm bg-background shadow-sm"
                  transition={{ type: "spring", bounce: 0.2, duration: 0.5 }}
        ...
        <div className="lg:col-span-4 flex flex-col gap-4 h-full">
          ...
          <div className="ms-card p-4 space-y-3 flex flex-col flex-grow">
            <h2 className="text-xs font-semibold text-foreground">
              {filterMode === "date"
                ? `Tambah Catatan (${format(new Date(currentDate), "d MMM")})`
                : "Tambah Catatan Baru"}
            </h2>
            <Textarea
              value={newNoteText}
              ...
              : (
                <AnimatePresence>
                  {displayedNotes.map((note) => {
                    ...
                    const priority = noteIsAdmin
                      ? (note as AdminNote).priority
                      : "normal";
                    const colorStyle = getStickyColor(
                      note.id,
                      noteIsAdmin,
                      priority,
                    );
                    const transform = noteTransforms[note.id] ||
                      { x: "50%", y: "50%", rot: 0, z: 1 };
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm flex items-center gap-2">
              ...
              {selectedNote && isAdminNote(selectedNote)
                ? "Catatan dari Admin"
                : "Catatan Harian"}
            </DialogTitle>
          </DialogHeader>
          {selectedNote && (
            <div className="space-y-3">
              {isAdminNote(selectedNote) && (
                <Badge
                  variant={selectedNote.priority === "important"
                    ? "destructive"
                    : "outline"}
                  className={`text-[9px] gap-1 ${
                    selectedNote.priority !== "important"
              ...
              {!isAdminNote(selectedNote) && (
                <Button
                  ...
                  onClick={() => {
                    setConfirmDelete(selectedNote.id);
                    setSelectedNote(null);
                  }}
                >
