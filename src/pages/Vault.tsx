...
const Vault = () => {
  ...
  if (canManage && !employeeId) {
    ...
    const filtered = linkSearch
      ? companyLinks.filter((l) => l.title.toLowerCase().includes(linkSearch.toLowerCase()) || l.url.toLowerCase().includes(linkSearch.toLowerCase()))
      : companyLinks;
    ...
    const handleAddSubmit = (e: React.FormEvent) => {
      ...
      addCompanyLink({
        title: linkTitle, url: linkUrl, icon: "Link", category: linkCategory,
        username: linkUsername, password: linkPassword, description: linkDescription,
        assignedTo: linkAssignMode === "all" ? "all" : selectedEmployees.join(","),
      });
      toast.success("Tautan ditambahkan");
      setShowAddDialog(false);
      setLinkTitle(""); setLinkUrl(""); setLinkCategory("Alat Pengembang");
      setLinkUsername(""); setLinkPassword(""); setLinkDescription("");
      setSelectedEmployees([]); setLinkAssignMode("all");
    };
    ...
    const handleEditSubmit = (e: React.FormEvent) => {
      ...
      updateCompanyLink(editingLinkId, {
        title: eTitle, url: eUrl, category: eCat,
        username: eUser, password: ePass, description: eDesc,
        assignedTo: eAssignMode === "all" ? "all" : selectedEmployees.join(","),
      });
    ...
    const openEdit = (link: typeof companyLinks[number]) => {
      ...
      setETitle(link.title || ""); setEUrl(link.url || ""); setECat(link.category || "");
      setEUser(link.username || ""); setEPass(link.password || ""); setEDesc(link.description || "");
      if (link.assignedTo === "all" || !link.assignedTo) { setEAssignMode("all"); setSelectedEmployees([]); }
      else { setEAssignMode("specific"); setSelectedEmployees(link.assignedTo.split(",")); }
      setShowEditDialog(true);
    };

    const renderAssignField = (mode: "all" | "specific", setMode: (m: "all" | "specific") => void) => (
      <div className="space-y-1">
        <Label className="text-xs">Tujuan</Label>
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-1.5 text-xs cursor-pointer">
            <input type="radio" checked={mode === "all"} onChange={() => setMode("all")} className="accent-primary" />
            <Users className="w-3 h-3" /> Semua Karyawan
          </label>
          <label className="flex items-center gap-1.5 text-xs cursor-pointer">
            <input type="radio" checked={mode === "specific"} onChange={() => setMode("specific")} className="accent-primary" />
            Pilih Karyawan
          </label>
        </div>
        {mode === "specific" && (
          <div className="border border-border rounded-md p-2 max-h-32 overflow-y-auto space-y-1 mt-2">
            {allEmployees.map(emp => (
              <label key={emp.id} className="flex items-center gap-2 text-xs cursor-pointer hover:bg-muted/50 px-1 py-0.5 rounded">
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <Button size="sm" className="gap-1 text-xs" onClick={() => setShowAddDialog(true)}>
        ...
        <div className="relative max-w-sm">
          ...
          <Input placeholder="Cari tautan..." value={linkSearch} onChange={(e) => setLinkSearch(e.target.value)} className="pl-9 h-9 text-sm" />
        </div>

        {filtered.length === 0 ? (
          <EmptyState icon={LinkIcon} title="Belum ada tautan" description="Tambahkan tautan untuk karyawan." compact />
        ) : (
          <div className="grid grid-cols-1 gap-3">
            {filtered.map((link, i) => {
              const catColor = CATEGORY_COLORS[link.category] || "bg-muted text-muted-foreground";
              const isExpanded = expandedLink === link.id;
              const hasCredentials = link.username || link.password;
              return (
                <motion.div key={link.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="ms-card-hover group">
                  <div className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0 flex-1 cursor-pointer" onClick={() => setExpandedLink(isExpanded ? null : link.id)}>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <Badge variant="secondary" className={`text-[9px] px-1.5 py-0 ${catColor}`}>{link.category}</Badge>
                          {hasCredentials && <Badge variant="outline" className="text-[9px] px-1.5 py-0 gap-0.5"><Lock className="w-2.5 h-2.5" /> Ada Akses</Badge>}
                          <Badge variant="outline" className="text-[9px] px-1.5 py-0 gap-0.5">
                            <Users className="w-2.5 h-2.5" /> {getAssignLabel(link.assignedTo)}
                          </Badge>
                        </div>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <Button size="icon" variant="ghost" className="w-7 h-7" onClick={() => copyToClipboard(link.url, `link-${link.id}`)}>
                        {copiedId === `link-${link.id}` ? <Check className="w-3.5 h-3.5 text-success" /> : <Copy className="w-3.5 h-3.5" />}
                      </Button>
                      <Button size="icon" variant="ghost" className="w-7 h-7" onClick={() => window.open(link.url, "_blank")}>
                        <ExternalLink className="w-3.5 h-3.5" />
                      </Button>
                      <Button size="icon" variant="ghost" className="w-7 h-7 opacity-0 group-hover:opacity-100" onClick={() => openEdit(link)}>
                        <Edit2 className="w-3.5 h-3.5" />
                      </Button>
                      <Button size="icon" variant="ghost" className="w-7 h-7 text-destructive opacity-0 group-hover:opacity-100" onClick={() => setConfirmDeleteLink(link.id)}>
                        <Trash2 className="w-3.5 h-3.5" />
                      </Button>
                    </div>
                  </div>
                  {isExpanded && hasCredentials && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="border-t border-border px-4 py-3 bg-muted/20 space-y-2">
                      {link.description && <p className="text-xs text-muted-foreground">{link.description}</p>}
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {link.username && (
                          <div>
                            <span className="text-[10px] text-muted-foreground uppercase tracking-wider flex items-center gap-1"><UserCircle className="w-3 h-3" /> Username</span>
                            <div className="flex items-center gap-1 mt-0.5">
                              <p className="text-foreground text-xs font-mono">{link.username}</p>
                              <Button size="icon" variant="ghost" className="w-6 h-6" onClick={() => copyToClipboard(link.username!, `linkuser-${link.id}`)}>
                                {copiedId === `linkuser-${link.id}` ? <Check className="w-3 h-3 text-success" /> : <Copy className="w-3 h-3" />}
                              </Button>
                            </div>
                          </div>
                        )}
                        {link.password && (
                          <div>
                            <span className="text-[10px] text-muted-foreground uppercase tracking-wider flex items-center gap-1"><Lock className="w-3 h-3" /> Password</span>
                            <div className="flex items-center gap-1 mt-0.5">
                              <p className="text-foreground text-xs font-mono">{visiblePasswords.has(`linkpw-${link.id}`) ? link.password : "••••••••"}</p>
                              <Button size="icon" variant="ghost" className="w-6 h-6" onClick={() => togglePassword(`linkpw-${link.id}`)}>
                                {visiblePasswords.has(`linkpw-${link.id}`) ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                              </Button>
                              <Button size="icon" variant="ghost" className="w-6 h-6" onClick={() => copyToClipboard(link.password!, `linkpw-${link.id}`)}>
                                {copiedId === `linkpw-${link.id}` ? <Check className="w-3 h-3 text-success" /> : <Copy className="w-3 h-3" />}
                              </Button>
                            </div>
                          </div>
                        )}
        ...
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
            <DialogHeader><DialogTitle className="text-sm">Tambah Tautan Baru</DialogTitle></DialogHeader>
            <form onSubmit={handleAddSubmit} className="space-y-3">
              <div className="space-y-1"><Label className="text-xs">Judul</Label><Input value={linkTitle} onChange={e => setLinkTitle(e.target.value)} placeholder="cth. Portal SDM" required /></div>
              <div className="space-y-1"><Label className="text-xs">URL</Label><Input value={linkUrl} onChange={e => setLinkUrl(e.target.value)} type="url" placeholder="https://..." required /></div>
              <div className="space-y-1"><Label className="text-xs">Kategori</Label><Input value={linkCategory} onChange={e => setLinkCategory(e.target.value)} placeholder="cth. SDM, Operasional" /></div>
              {renderAssignField(linkAssignMode, setLinkAssignMode)}
              <div className="space-y-1"><Label className="text-xs">Keterangan</Label><Textarea value={linkDescription} onChange={e => setLinkDescription(e.target.value)} placeholder="Catatan..." rows={2} /></div>
              <div className="border border-border rounded-lg p-3 space-y-3 bg-muted/30">
                <p className="text-xs font-medium text-foreground flex items-center gap-1.5"><Lock className="w-3 h-3 text-primary" /> Kredensial Akses</p>
                <div className="space-y-1"><Label className="text-xs">Username / Email</Label><Input value={linkUsername} onChange={e => setLinkUsername(e.target.value)} /></div>
                <div className="space-y-1"><Label className="text-xs">Password</Label><Input value={linkPassword} onChange={e => setLinkPassword(e.target.value)} /></div>
              </div>
              <Button type="submit" className="w-full text-xs">Tambah Tautan</Button>
            </form>
          </DialogContent>
        </Dialog>

        {/* Edit Dialog */}
        <Dialog open={showEditDialog} onOpenChange={o => { if (!o) { setShowEditDialog(false); setEditingLinkId(null); } }}>
          <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
            <DialogHeader><DialogTitle className="text-sm">Edit Tautan</DialogTitle></DialogHeader>
            <form onSubmit={handleEditSubmit} className="space-y-3">
              <div className="space-y-1"><Label className="text-xs">Judul</Label><Input value={eTitle} onChange={e => setETitle(e.target.value)} required /></div>
              <div className="space-y-1"><Label className="text-xs">URL</Label><Input value={eUrl} onChange={e => setEUrl(e.target.value)} type="url" required /></div>
              <div className="space-y-1"><Label className="text-xs">Kategori</Label><Input value={eCat} onChange={e => setECat(e.target.value)} /></div>
              {renderAssignField(eAssignMode, setEAssignMode)}
              <div className="space-y-1"><Label className="text-xs">Keterangan</Label><Textarea value={eDesc} onChange={e => setEDesc(e.target.value)} rows={2} /></div>
              <div className="border border-border rounded-lg p-3 space-y-3 bg-muted/30">
                <p className="text-xs font-medium text-foreground flex items-center gap-1.5"><Lock className="w-3 h-3 text-primary" /> Kredensial</p>
                <div className="space-y-1"><Label className="text-xs">Username</Label><Input value={eUser} onChange={e => setEUser(e.target.value)} /></div>
                <div className="space-y-1"><Label className="text-xs">Password</Label><Input value={ePass} onChange={e => setEPass(e.target.value)} /></div>
              </div>
              <Button type="submit" className="w-full text-xs">Simpan Perubahan</Button>
            </form>
          </DialogContent>
        </Dialog>

        <ConfirmDialog open={!!confirmDeleteLink} onOpenChange={o => { if (!o) setConfirmDeleteLink(null); }} title="Hapus tautan ini?" description="Tautan akan dihapus permanen." variant="destructive" confirmText="Hapus" onConfirm={() => { if (confirmDeleteLink) { removeCompanyLink(confirmDeleteLink); setConfirmDeleteLink(null); toast.success("Tautan dihapus"); } }} />
      </motion.div>
    );
  }
